...
#
# This example shows how to dynamically add hosts and switches to a running emulation.
# Due to technical limitations it is NOT possible to create a link between a switch and a host if these are emulated at DIFFERENT workers
...
exp.addSwitch("s2",dpid=Tools.makeDPID(2),wid=2) # Enforce placement of s2 on Worker 2. Otherwise random worker would be chosen
print "adding hosts h3 and h4 on worker 2..."
exp.addHost("h3",ip=Tools.makeIP(3),max=Tools.makeMAC(3),pos="s2") # Enforce placement of h3 on worker of s2. Remember: we cannot have tunnels between hosts and switches
exp.addHost("h4",ip=Tools.makeIP(4),max=Tools.makeMAC(4),pos="s2")
exp.addLink("s1","s2",autoconf=True) # autoconf parameter configures link-attachment etc for us
...
print exp.get("h3").cmd("ping -c5 10.0.0.4") # show network connectivity of new hosts
