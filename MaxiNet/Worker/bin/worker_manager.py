...
group.add_argument("--stop", help="stop Worker daemons on remote hosts",
                   action="store_true")
parser.add_argument("--ns", help="nameserver to use", required=True,
                    type=str, nargs=1, metavar="NAMESERVER")
parser.add_argument("--hmac", help="hmac key to use", type=str, nargs=1,
                    metavar="KEY")
parser.add_argument("hosts", help="use these hosts",
                    nargs='+', metavar=("HOST1", "HOST2"), type=str)
parser.add_argument("--debugPyro", help="Set Pyro to debug level",
                    action="store_true")
parser.add_argument("--keepScreenOpenOnError",
                    help="Keep the screen sessions", action="store_true")
...
def stop(hn):
    ...
    path = subprocess.check_output(["whereis", "MaxiNetServer"])\
                .split(":")[1].strip()
...
for vm in hosts:
    ...
    if (args.start):
        if (subprocess.call(chkcmd, stdout=dnull, shell=True) == 0):
            ...
            stop(hn)
        threads.append(threading.Thread(None, start, None,
                                        (hn, debugPyro,
                                         keepScreenOpenOnError)))
        threads[-1].start()
    if (args.stop):
        if (subprocess.call(chkcmd, shell=True, stdout=dnull) == 1):
            print "ignoring " + vm + " as there seems to be no running " +\
                  "instance"
            continue
