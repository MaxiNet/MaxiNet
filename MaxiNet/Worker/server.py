...
class PyroServer():
    def __init__(self, host=None, nsaddress=None, nsport=9090):
        ...
        self.nameserver = Pyro4.locateNS(nsaddress, nsport)
...
def main():
    ...
    server.register_obj(MininetCreator(),
                        'worker{0}.mnCreator'.format(workerID))
    server.register_obj(CmdListener(workerDir),
                        'worker{0}.cmd'.format(workerID))
