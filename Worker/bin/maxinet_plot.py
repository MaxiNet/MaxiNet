...
oldversion = False
if(int(mpl.__version__.split(".")[0])<=1 and int(mpl.__version__.split(".")[1])<2): #framealpha is only availabe for matplotlib version >= 1.2.0
    oldversion = True 
    
if(not len(sys.argv) in [2,3]):
    print "ERROR: Invalid number of parameters.\nSyntax: "+sys.argv[0]+" LOGFILE_FOLDER [IMAGE_FILENAME] "
    sys.exit(1)
folder = sys.argv[1]
...
for filename in memlogs:
    ...
    print axis_x
    #ax[graph].plot(axis_x,axis_y_stack,label=workerHN)
    ax[graph].fill_between(axis_x,0,axis_y_stack[0,:],facecolor="#CC6666")
    ax[graph].fill_between(axis_x,axis_y_stack[0,:],axis_y_stack[1,:], facecolor="#1DACD6")
    ax[graph].fill_between(axis_x,axis_y_stack[1,:],axis_y_stack[2,:], facecolor="#6E5160")
    ax[graph].set_ylabel("Memory [MB]")
    ax[graph].set_ylim(bottom=0)
    if(graph == numworkers+1):
        ax[graph].set_xlabel("Time [s]")
    if oldversion:
        ax[graph].legend([mpl.patches.Rectangle((0,0),1,1,fc="#CC6666"),mpl.patches.Rectangle((0,0),1,1,fc="#1DACD6"),mpl.patches.Rectangle((0,0),1,1,fc="#6E5160")],["MemFree","Buffers","Cached"],loc="best")
    else:
        ax[graph].legend([mpl.patches.Rectangle((0,0),1,1,fc="#CC6666"),mpl.patches.Rectangle((0,0),1,1,fc="#1DACD6"),mpl.patches.Rectangle((0,0),1,1,fc="#6E5160")],["MemFree","Buffers","Cached"],framealpha=0.5, loc="best")
